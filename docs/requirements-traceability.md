...
| QR check-in | HTTPS QR link, login-preserved intent, explicit confirmation and secured API transition | Playwright `qr-check-in.spec.ts` in desktop and mobile Chromium; invalid and repeated check-in backend tests | Complete for online-first scope |
